...
keypoints=dlmread('keypoints.txt');
    ...
    if strcmp(mode,'gaussians')
	    ...
	    bb_id=str2num(imagename(7:end));
            ...
            for j=1:size(input,3)
                ...
                %    [r, c]=find(input(:,:,j)==1);
                %    btemp(1)=round(mean(r,1));
                %    btemp(2)=round(mean(c,1));
                %    if(input(round(btemp(1)),round(btemp(2)),j)==0)
                %        rand_ind=randi(size(r,1),1,1);
                %        btemp(1)=r(rand_ind);
                %        btemp(2)=c(rand_ind);
                %    end
    ...
    elseif (strcmp(mode,'grabcut'))
        ...
        for i=max(1,start_id):min(size(C,1),end_id)
			...
			while(sum(sum(fixedbg))>2800)
                   ...
                   while(~success && iter<3)
                    try
                        ...
                        sprintf('GCAlgo called')  %Johann's amendment
                        success = 1;
                    catch
                      iter=iter+1;
            ...
            if strcmp(mode,'proposed')
                data=backup;
		data=uint8(data);
                save(strcat(weak_groundtruth_dir,strrep(C(i).name,'_blob_0.mat','_binary_multilabel_backup.mat')),'data')
            end
	end
     else
