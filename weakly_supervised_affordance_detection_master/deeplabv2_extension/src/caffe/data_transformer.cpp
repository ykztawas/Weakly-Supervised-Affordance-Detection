...
void DataTransformer<Dtype>::TransformImgAndSeg(
    ...
    const int ignore_label, bool ML) {
    ...
    if (scale != 1) {
      ...
      if(!ML)
      {
		 ...
		 cv::INTER_NEAREST);
          ...
          vector<cv::Mat> seg_channels2;
          cv::split(cv_img_seg[1], seg_channels2);
	  for(int c=0;c<seg_channels2.size();c++)
          {
               cv::resize(seg_channels2[c], seg_channels2[c], cv::Size(img_width, img_height), 0, 0, 
		 cv::INTER_NEAREST);
          }

          cv::merge(seg_channels2, cv_cropped_seg);
      }
    } else {
  ...
  if (pad_height > 0 || pad_width > 0) {
    ...
    } else // crop image for every segmentation channel!
    {
        vector<cv::Mat> seg_channels;
        cv::split(cv_cropped_seg, seg_channels);

        for(int c=0;c<seg_channels.size();c++)
        {
            cv::copyMakeBorder(seg_channels[c], seg_channels[c], 0, pad_height,
                  0, pad_width, cv::BORDER_CONSTANT,
                       cv::Scalar(0));
        }

        cv::merge(seg_channels, cv_cropped_seg);
    }
  ...
  if (crop_width && crop_height) {
    ...
    if(!ML){
      ...
      vector<cv::Mat> seg_channels3;
      cv::split(cv_cropped_seg, seg_channels3);

      for(int c=0;c<seg_channels3.size();c++)
      {
           seg_channels3[c]=seg_channels3[c](roi);
      }
      cv::merge(seg_channels3, cv_cropped_seg);
    }
  }
...
//  int control_sum=0;
 ...
 //       LOG(INFO) << "image pixel in data transformer\t" << transformed_data[top_index];
//        LOG(INFO) << "channel, height, width" << c<< "\t" << h << "\t" << w ;
      }
      ...
      {
//LOG(INFO) << "Entered multilable mode"; //Johann's amendment
          for(int lc = 0; lc < label_channels; lc++)   //Johann's amendment
          {
              if(do_mirror)
              {
                top_index = (lc * data_height + h) * data_width + (data_width - 1 - w);
              } else
              {
                top_index = (lc * data_height + h) * data_width + w;
              }

              Dtype pixel = static_cast<Dtype>(label_ptr[label_index++]);
	      
//	      CHECK_GT(0.0001, pixel*pixel*(1-pixel)*(1-pixel)); //Johann's amendment
//	      control_sum_labels+=static_cast<float>(pixel);  // Johann's amendment
              transformed_label[top_index] = pixel;
//              control_sum+=pixel*h;
//              control_sum+=pixel*w;
          }
