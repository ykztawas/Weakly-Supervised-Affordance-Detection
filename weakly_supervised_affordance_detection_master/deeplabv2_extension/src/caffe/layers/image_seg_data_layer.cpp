...
void ImageSegDataLayer<Dtype>::DataLayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if(label_type == ImageDataParameter_LabelType_MATPIXEL)
  {
      ...
      cv::Mat segImg = ReadMatFileToCVMat(matPath, new_height, new_width, "data");
  ...
  if (crop_width > 0 && crop_height > 0) {
    ...
    //label
    top[1]->Reshape(batch_size, label_channels, crop_height, crop_width);
    this->transformed_label_.Reshape(batch_size, label_channels, crop_height, crop_width);
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
      this->prefetch_[i].label_.Reshape(batch_size, label_channels, crop_height, crop_width);
    }
  } else {
    ...
    this->transformed_label_.Reshape(batch_size, label_channels, height, width);
    for (int i = 0; i < this->PREFETCH_COUNT; ++i) {
...
void ImageSegDataLayer<Dtype>::load_batch(Batch<Dtype>* batch) {
  ...
  double segmentation_read_time = 0;
  double trans_time = 0;
  CPUTimer timer;
  CPUTimer image_timer;
...
//    LOG(INFO) << "read image: \t" << root_folder + lines_[lines_id_].first;
    ...
    segmentation_read_time += image_timer.MicroSeconds();
...
//    const Dtype* label_copy = this->transformed_label_.cpu_data(); //changed by Johann
//    int cn = this->transformed_label_.count(); //changed by Johann
//    float control_sum=0;   //changed by Johann
//    for(int i=0; i<cn; i++){    //changed by Johann
//	control_sum += label_copy[i];
   //   CHECK_GE(data[i], -255) << i;    //changed by Johann
   //   CHECK_LE(data[i], 255) << i;    //changed by Johann
   //   CHECK_EQ(data[i], data[i]);    //changed by Johann
//    }    //changed by Johann
//     LOG(INFO) << "sum of labels" << control_sum ;  //changed by Johann
  ...
  DLOG(INFO) << "     image read time: " << image_read_time / 1000 << " ms.";
  DLOG(INFO) << "     segmentation read time: " << segmentation_read_time / 1000 << " ms.";
